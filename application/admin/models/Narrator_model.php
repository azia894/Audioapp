...
class narrator_model extends CI_Model{
	
	private $tbl_name='aud_narrator';
	...
	function getDetailsByName($nar_name){
		$query = "select * from $this->tbl_name where  nar_name=?";
		$q = $this->db->query($query,array($nar_name));		
		$num =  $q->num_rows();
		$data="";
		if ($num > 0) {
			$data = $q->result_array();		
